...
const payments = DEMO_DATASET.adminPayments || [];
...
const minishopSql = `${header(
  "Minishop PostgreSQL",
  `users=${users.length}, subscriptions=${subscriptions.length}, payments=${payments.length}, logs=${payload.logs.length}, tickets=${payload.tickets.length}, messages=${supportMessages.length}, promos=${payload.promos.length}, ads=${payload.ads.length}`
)}BEGIN;
...
INSERT INTO payments (
    ...
    promo_conflict_override, promo_usage_restored, created_at, updated_at
)
SELECT
