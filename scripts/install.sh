...
EMAIL_AUTH_SECRET_VALUE=""
PUBLIC_APP_URL_VALUE=""
EMAIL_LOGIN_ENABLED_VALUE=""
SMTP_HOST_VALUE=""
SMTP_PORT_VALUE=""
SMTP_USERNAME_VALUE=""
SMTP_PASSWORD_VALUE=""
...
prompt_common_env() {
    ...
    telegram_default=0
    [ -n "$detected_bot_token" ] && telegram_default=1
    if confirm "Включить Telegram-бота?" "$telegram_default"; then
        TELEGRAM_ENABLED_VALUE="True"
        prompt_value "Токен Telegram бота" "$detected_bot_token" 1 1 "" "$detected_bot_token_prefilled"
        BOT_TOKEN_VALUE="$PROMPT_VALUE"
    else
        TELEGRAM_ENABLED_VALUE="False"
        BOT_TOKEN_VALUE=""
    fi
    ...
    if [ "$TELEGRAM_ENABLED_VALUE" = "True" ]; then
        prompt_value "Старые Telegram ID администраторов для однократного импорта (пусто = пропустить)" "$detected_admin_ids" 0 0 "" "$detected_admin_ids_prefilled"
    ...
    EMAIL_AUTH_SECRET_VALUE="$(env_get EMAIL_AUTH_SECRET "")"
    if [ -z "$EMAIL_AUTH_SECRET_VALUE" ]; then
        EMAIL_AUTH_SECRET_VALUE="$(secret_hex 32)"
    fi
    EMAIL_LOGIN_ENABLED_VALUE="$(env_get EMAIL_LOGIN_ENABLED True)"
    if [ "$TELEGRAM_ENABLED_VALUE" = "False" ]; then
        EMAIL_LOGIN_ENABLED_VALUE="True"
        prompt_value "SMTP host для входа по email" "$(env_get SMTP_HOST '')" 1 0 ""
        SMTP_HOST_VALUE="$PROMPT_VALUE"
        prompt_value "Email отправителя" "$(env_get SMTP_FROM_EMAIL '')" 1 0 ""
        SMTP_FROM_EMAIL_VALUE="$PROMPT_VALUE"
    else
        SMTP_HOST_VALUE="$(env_get SMTP_HOST '')"
        SMTP_FROM_EMAIL_VALUE="$(env_get SMTP_FROM_EMAIL '')"
    fi
    SMTP_PORT_VALUE="$(env_get SMTP_PORT 587)"
    SMTP_USERNAME_VALUE="$(env_get SMTP_USERNAME '')"
    SMTP_PASSWORD_VALUE="$(env_get SMTP_PASSWORD '')"
    if [ -n "$SMTP_HOST_VALUE" ]; then
        prompt_value "SMTP port" "$SMTP_PORT_VALUE" 1 0 ""
        SMTP_PORT_VALUE="$PROMPT_VALUE"
        prompt_value "SMTP username (пусто = без авторизации)" "$SMTP_USERNAME_VALUE" 0 0 ""
        SMTP_USERNAME_VALUE="$PROMPT_VALUE"
        prompt_value "SMTP password (пусто = без авторизации)" "$SMTP_PASSWORD_VALUE" 0 1 ""
        SMTP_PASSWORD_VALUE="$PROMPT_VALUE"
    fi
    ...
    PUBLIC_APP_URL_VALUE="$(env_get PUBLIC_APP_URL "$MINIAPP_PUBLIC_URL_VALUE")"
}
...
display_env_summary() {
    ...
    show_env_value TELEGRAM_ENABLED "$TELEGRAM_ENABLED_VALUE"
    ...
    show_env_value EMAIL_AUTH_SECRET "$EMAIL_AUTH_SECRET_VALUE"
    show_env_value PUBLIC_APP_URL "$PUBLIC_APP_URL_VALUE"
    show_env_value EMAIL_LOGIN_ENABLED "$EMAIL_LOGIN_ENABLED_VALUE"
    show_env_value SMTP_HOST "$SMTP_HOST_VALUE"
    show_env_value SMTP_PORT "$SMTP_PORT_VALUE"
    show_env_value SMTP_USERNAME "$SMTP_USERNAME_VALUE"
    show_env_value SMTP_PASSWORD "$SMTP_PASSWORD_VALUE"
...
render_env_file() {
    ...
    printf '\n# Telegram\n' >> "$output"
    env_line TELEGRAM_ENABLED "$TELEGRAM_ENABLED_VALUE" "$output"
    ...
    env_line EMAIL_AUTH_SECRET "$EMAIL_AUTH_SECRET_VALUE" "$output"
    env_line PUBLIC_APP_URL "$PUBLIC_APP_URL_VALUE" "$output"
    env_line EMAIL_LOGIN_ENABLED "$EMAIL_LOGIN_ENABLED_VALUE" "$output"
    env_line SMTP_HOST "$SMTP_HOST_VALUE" "$output"
    env_line SMTP_PORT "$SMTP_PORT_VALUE" "$output"
    env_line SMTP_USERNAME "$SMTP_USERNAME_VALUE" "$output"
    env_line SMTP_PASSWORD "$SMTP_PASSWORD_VALUE" "$output"
    env_line SMTP_FROM_EMAIL "$SMTP_FROM_EMAIL_VALUE" "$output"
