...
def test_upgrade_target_preserves_the_source_database_volume(tmp_path: Path) -> None:
    ...
    assert upgraded["REMNAWAVE_STAND_PRESET"] == "upgrade-2.8.1-to-3.3.2"
    assert upgraded["REMNAWAVE_DEV_VERSION"] == "3.3.2"
    ...
    assert upgraded["REMNAWAVE_UPGRADE_TO"] == "3.3.2"
...
def test_dev_mock_data_profile_is_explicit_idempotent_and_safe() -> None:
    ...
    panel_seed = (DEV_DIR / "seed-remnawave-mock-data.sql").read_text(encoding="utf-8")
    compose = (REPO_ROOT / "docker-compose.remnawave-dev.yml").read_text(encoding="utf-8")
    scripts = json.loads((REPO_ROOT / "package.json").read_text(encoding="utf-8"))["scripts"]

    assert seed.startswith("-- Generated by scripts/generate_dev_mock_seed.mjs")
    assert seed.rstrip().endswith("COMMIT;")
    assert "users=370, subscriptions=257, payments=485, reversible_payments=184" in seed
    assert "tmp/postgres.dump (2026-05-28T06:00:00Z, anonymized)" in seed
    assert "telegram_notifications_status = 'blocked'" in seed
    assert "telegram_id = NULL" in seed
    assert "skip_notifications = true" in seed
    assert "client.example" in seed
    assert "910030001" in seed
    assert "ON CONFLICT (user_id) DO UPDATE" in seed
    assert "ON CONFLICT (ticket_id) DO UPDATE" in seed
    assert "ON CONFLICT (payment_id) DO UPDATE" in seed
    assert "Reversible docs-demo fixture" in seed
    assert "fulfillment_before_snapshot = payloads.before_snapshot::text" in seed
    assert "ON CONFLICT (broadcast_id) DO UPDATE" in seed
    assert panel_seed.startswith("-- Generated by scripts/generate_dev_mock_seed.mjs")
    assert "panel_users=253" in panel_seed
    assert "mini-shop-docs-demo" in panel_seed
    ...
    assert "seed-remnawave-mock-data.sql:/seed/seed-remnawave-mock-data.sql:ro" in compose
