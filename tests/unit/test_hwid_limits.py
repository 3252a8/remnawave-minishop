...
class ResolveHwidDeviceLimitsTests(unittest.IsolatedAsyncioTestCase):
    async def test_subscription_override_wins_and_adds_extras(self):
        ...
        sub = SimpleNamespace(hwid_device_limit=5, hwid_device_limit_is_override=True)
    ...
    async def test_tariff_change_replaces_stored_base_before_adding_extras(self):
        service = _service()
        service._active_hwid_extra_devices_for_sub = AsyncMock(return_value=2)
    ...
    async def test_higher_inherited_base_is_reduced_by_tariff(self):
        service = _service()
        service._active_hwid_extra_devices_for_sub = AsyncMock(return_value=2)
    ...
    async def test_inherited_unlimited_becomes_finite_with_tariff(self):
        service = _service()
        service._active_hwid_extra_devices_for_sub = AsyncMock(return_value=2)
        sub = SimpleNamespace(hwid_device_limit=0)
        tariff = SimpleNamespace(hwid_device_limit=3)
    ...
    async def test_inherited_finite_limit_becomes_unlimited_with_tariff(self):
        service = _service()
        service._active_hwid_extra_devices_for_sub = AsyncMock(return_value=2)
        sub = SimpleNamespace(hwid_device_limit=3)
        tariff = SimpleNamespace(hwid_device_limit=0)

        limits = await service._resolve_hwid_device_limits(None, sub, tariff)

        self.assertEqual(limits, HwidDeviceLimits(base=0, extra=2, effective=0))
