...
- `main` только синхронизируется с upstream `main`; разработка и деплой из неё не ведутся.
- Разработка идёт в `dev`; upstream-обновления попадают в неё через `git merge main`.
- Production-деплой идёт из `prod`; релиз попадает в неё через `git merge dev`.
- Ветка от `dev` (или как укажет мейнтейнер); PR'ы целятся в `dev`/`prod` и должны проходить все
  гейты раздела 1.
