...
- встроиться в уже запущенный reverse proxy на сервере: помимо схемы eGames
  wizard находит любые запущенные Nginx/Caddy контейнеры, подключает их к
  ...
  проверяет конфиг (`nginx -t` / `caddy validate`) и откатывает изменения при
  ошибке;
...
| `Профиль деплоя` | Как приложение будет опубликовано наружу. | `Caddy HTTPS` для нового публичного сервера; `Nginx HTTPS`, если уже управляете сертификатами; `Pangolin/Newt`, если входящие порты закрыты; `Без прокси`, если TLS завершается внешней платформой; профиль существующего reverse proxy, если на этом же хосте уже работает Remnawave через eGames или любой другой Nginx/Caddy контейнер. |
| `Подключение к существующему reverse proxy` | Для профиля существующего прокси: схема eGames (unix socket), универсальное подключение к любому запущенному Nginx/Caddy контейнеру или пропуск. | Схему eGames — для установки Remnawave скриптом eGames; универсальное подключение — для остальных случаев. Wizard сам определит host/bridge-сеть контейнера, переиспользует найденный `ssl_certificate` и откатит конфиг, если `nginx -t`/`caddy validate` не пройдет. |
...
| `WEBHOOK_HOST` / `MINIAPP_HOST` | Публичные домены без `https://`, пути и порта. | Например `webhooks.example.com` и `app.example.com`. Для Caddy/Nginx/eGames DNS должен указывать на сервер или на прокси перед ним. |
| `HTTP_BIND` / `HTTPS_BIND` | На каком IP и портах Caddy/Nginx слушает входящий HTTP/HTTPS. | Обычно `0.0.0.0:80` и `0.0.0.0:443`. IP без порта указывать нельзя. |
...
Minishop уже установлен с другим профилем (Caddy, Nginx, no-proxy, eGames),
