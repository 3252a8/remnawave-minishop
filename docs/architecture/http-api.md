...
- `/api/admin/*` — API админки внутри Web App. Все маршруты требуют сессию
  аккаунта с действующей ролью `owner` или `admin`. Управление ролями доступно
  только владельцу через `/api/admin/roles`.
...
после проверки сессии дополнительно сверяется действующая роль аккаунта в базе.
Подтверждённый email-аккаунт может получить права без Telegram ID.
