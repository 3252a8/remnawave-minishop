...
- пользователи Telegram и email-only аккаунты, username, подтвержденные email,
  Google/Yandex/VK OAuth identities, Remnawave UUID и метаданные профиля;
- старые referral codes и связи рефералов;
- текущий баланс `points` как стартовая запись пользовательского баланса;
- подписки, сроки, лимиты трафика, HWID/device limit, UUID подписок панели,
  managed squads и индивидуальные internal/external squad overrides;
- платежи и статусы платежей;
- коды активации на дни подписки или обычный трафик и их активации, если
  таблицы есть в source DB;
- рекламные ссылки `ad_links` и первая атрибуция пользователей;
...
Реферальные связи остаются обычными рефералами. Миграция не создает из них
партнерские профили или заявки партнерской программы.

Пароли Remnashop не копируются: их хэши не считаются совместимым контрактом.
Подтвержденные адреса и OAuth identities позволяют сопоставить существующий
аккаунт; пользователи только с email получают стабильный технический `user_id`.

### Валюта баланса

Один `point` переносится как одна целая единица `settings.default_currency`
Remnashop. Для RUB/USD это означает умножение на 100 minor units, а для Telegram
Stars (`XTR`) scale равен 0 и значение переносится без умножения. Importer не
суммирует строки `referral_rewards`, потому что итог уже хранится в `users.points`.

Wizard показывает выбранную валюту после dry-run и, если есть ненулевые
балансы, записывает в `.env` `USER_BALANCE_CURRENCY` и включает
`USER_BALANCE_ENABLED`. Валюту можно явно переопределить через
`REMNASHOP_BALANCE_CURRENCY` или CLI-флаг `--balance-currency`. Если валюта не
определяется либо повторный запуск обнаруживает отличающуюся стартовую запись,
применение блокируется и транзакция откатывается.

Не переносятся автоматически история рассылок, device-only коды, бессрочные
трафиковые коды и постоянные personal/purchase discounts: их семантика не имеет
безопасного прямого соответствия текущей модели Minishop. Они отражаются в
сводке и config plan для ручной проверки.
...
7. При необходимости задаете валюту баланса; пустой ответ использует
   `settings.default_currency` исходной базы.
8. Wizard запускает проверку без записи (`dry-run`), показывает краткую сводку
   и сохраняет полный JSON/raw-вывод в `.installer/remnashop-dry-run-summary.json`.
9. После подтверждения `y` importer применяет изменения. У вопроса применения
   дефолт `Y`, поэтому Enter после успешной проверки означает "применить";
   `n` остановит миграцию без записи.
10. После применения wizard сохраняет apply-сводку, обновляет webhook Remnawave
...
`--only users,referrals,balances,promocodes,advertising`.
