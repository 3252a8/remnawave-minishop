...
Для ручной проверки разных состояний интерфейса можно вместо обычного запуска
включить дополнительный профиль с мок-данными:

```powershell
npm run dev:stand:config:mocks
npm run dev:stand:up:mocks
```
...
При запуске `dev:stand:up:mocks` добавляются ещё три dev-only пользователя без
Telegram ID, чтобы тестовые действия не могли отправить сообщение реальному
получателю:

| User ID | Email | Сценарий |
| --- | --- | --- |
| `910000004` | `mock.new@example.com` | новый пользователь без подписки |
| `910000005` | `mock.banned@example.com` | заблокированный пользователь |
| `910000006` | `mock.email@example.com` | пользователь только с email |

Во всех шести аккаунтах из таблиц выше пароль `Minishop-Dev-2026!`. Мок-профиль
также создаёт тикеты в состояниях `open`, `awaiting_admin`, `awaiting_user`,
`resolved` и `closed`, платежи с успешным, ожидающим, отменённым и ошибочным
результатом, активные и истёкшие промокоды, а также завершённые, запланированные
и ошибочные рассылки. Запланированная рассылка установлена на 2099 год, адреса
используют `example.com`, а Telegram destinations являются dev-заглушками.

`deploy/dev/seed-minishop-mock-data.sql` использует зарезервированные диапазоны
ID и при повторном запуске обновляет только эти записи. Обычные пользователи,
тикеты и данные, созданные вручную, не удаляются.
