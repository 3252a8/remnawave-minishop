...
публичный репозиторий с готовым ZIP, закрепляет Ed25519-ключ издателя по заранее полученному
SHA-256 отпечатку, затем устанавливает и включает пакет. Предпросмотр не исполняет код.
...
`frontend/`. Поле `files` перечисляет SHA-256 каждого payload-файла. Подпись охватывает
...
```json
{"schema_version":1,"artifact":"dist/example-plugin.zip","sha256":"<64 lowercase hex characters>"}
```
...
загружать пакет с неизвестным ключом, использовать `.pth`, вложенные wheels, пути вне ZIP или
