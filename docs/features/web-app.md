...
- смену тарифа, обычную докупку трафика и докупку premium-трафика при настроенном каталоге тарифов; предложение докупки по умолчанию появляется после 80% расхода соответствующего лимита, если у тарифа не включен переключатель «Докупка доступна всегда» (подробнее: [когда показывается докупка](tariff-purchases.md#когда-показывается-докупка-трафика));
...
[докупки при оформлении](tariff-purchases.md#докупки-при-оформлении-подписки).
