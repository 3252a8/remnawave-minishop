...
умолчанию и поддерживает ровно один источник данных за раз. После включения пункт статуса
появляется в настройках Mini App. Карточка на главном экране остаётся выключенной, пока отдельно
не включён `SERVER_STATUS_SHOW_ON_HOME`:
...
Чтобы дополнительно показывать компактную карточку на главном экране, установите
`SERVER_STATUS_SHOW_ON_HOME=True`. Этот флаг не включает сам источник и не скрывает пункт статуса
из настроек.
...
- проверьте `SERVER_STATUS_ENABLED=True` и точное значение источника: `url`, `uptime-kuma` или
  `xray-checker`;
- если отсутствует только карточка на главном экране, проверьте
  `SERVER_STATUS_SHOW_ON_HOME=True`; по умолчанию статус доступен только через настройки;
