...
Существующие папки в `WEBAPP_THEMES_DIR` продолжают работать с прежним `theme.json` и CSS.
...
В библиотеке такая тема отмечается как установленная на сервере, без версии. Если скриншота
нет, карточка показывает пояснение; предпросмотр и активация остаются доступны. Отсутствие
или ошибка картинки не мешают работе темы.

Для старых тем предпросмотр допускает локальные ссылки с параметрами кеширования. Внешние
шрифты, изображения, недоступные ресурсы и неподдерживаемые CSS-правила в изолированном
предпросмотре пропускаются. Исходный CSS работающего ЛК при этом не изменяется.
...
Core принимает ZIP до 20 МиБ; HTTP-запрос немного больше из-за multipart-обрамления.
