...
При импорте разрешены `png`, `jpg`, `jpeg`, `gif`, `webp`, `ico` и локальные `woff`, `woff2`, `ttf`, `otf`. Один ресурс — до 10 МиБ. SVG сохраняет поддержку только для доверенных тем, установленных вручную. Для распространяемого пакета используйте относительные ссылки на локальные ресурсы; внешние `@import` и URL отклоняются.
