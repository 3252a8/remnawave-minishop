...
Готовые темы лежат в `backend/bot/app/web/themes`: `dark`, `windows95`, `ascii`. Светлый режим живет как вариант темы `dark`; ключ `light` оставлен только как скрытый алиас для старых настроек. При первом запуске темы копируются в `WEBAPP_THEMES_DIR`, по умолчанию `data/themes`.
