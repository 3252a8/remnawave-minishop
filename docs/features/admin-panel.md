...
админке доступны новые режимы промокодов: скидка в процентах, множитель срока подписки,
множитель выдаваемого трафика, бонусные дни сразу или после оплаты, область применения,
минимальный срок/объем покупки и история активаций.
...
- **Докупки**: обычные пакеты докупки трафика для `period` и переключатель **«Докупка доступна всегда»** (`topup_always_available`); по умолчанию докупка появляется у пользователя после 80% расхода лимита. Для `traffic` отдельные докупки не нужны, пользователь повторно покупает пакеты из `traffic_packages`;
- **Premium**: названия premium-раздела RU/EN, premium Internal Squads, месячный premium-лимит, RUB/Stars пакеты premium-докупки и отдельный переключатель **«Докупка доступна всегда»** (`premium_topup_always_available`) для premium-трафика;
