...
[разделе о докупках при оформлении](tariff-purchases.md#докупки-при-оформлении-подписки).
...
Когда включен провайдер Tribute, на вкладке **«Цены»** доступна кнопка **«Подтянуть из Tribute»**. Она читает каталог Creator API через `/api/admin/tariffs/tribute/catalog` тем же `TRIBUTE_API_KEY`: выбор подписки подставляет `subscription_id` и `period_id` во все периоды, выбор Digital Product — `product_id` и ссылку товара. Загруженный каталог одновременно сверяется с тарифом и показывает расхождения цены и валюты, а также ID, которых больше нет в Tribute. Запрос выполняется только по нажатию кнопки. Подробности режима — [Платежи](payments/tribute.md).
