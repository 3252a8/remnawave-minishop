...
разовые тестовые способы только аккаунтам с ролью администратора; тестовый webhook не может
...
Wata подключается как отдельный провайдер с bearer token, платежными ссылками,
рекуррентными подписками и опциональной проверкой подписи webhook. Рекуррентный режим
использует расписание на стороне Wata: после первой оплаты Wata сама списывает фиксированную
сумму и присылает новые транзакции в тот же webhook.
...
8. Для автоплатежей запросите включение подписок на терминале Wata и включите
   `WATA_SUBSCRIPTION_ENABLED`.
9. При необходимости ограничьте число списаний через `WATA_SUBSCRIPTION_MAX_PERIODS`.

### Ограничения

- `WATA_LINK_TTL_MINUTES` должен быть от `15` до `43200`.
- Автоплатёж доступен только для оформления подписки. Отдельные покупки трафика,
  премиального трафика и устройств остаются разовыми платежами.
- Гибкие лимиты, выбранные вместе с новой подпиской, входят в фиксированную сумму каждого
  списания. Изменение лимитов с доплатой за неполный первый период для Wata недоступно:
  первая сумма не должна отличаться от последующих.
- Тариф, базовые лимиты, гибкие лимиты и цена фиксируются в момент согласия. Изменение
  настроек тарифа не меняет уже созданный автоплатёж; для новых условий пользователь
  оформляет новую подписку Wata.
- При замене сначала подтверждается новый автоплатёж, затем Minishop завершает старый в
  Wata. При ручной смене тарифа старый автоплатёж завершается до применения нового тарифа;
  если Wata не подтвердила завершение, смена не выполняется.
- Для создания Wata-ссылки пользователь вводит email и телефон. Minishop передаёт их Wata,
  но не сохраняет в своей базе.
- Промокоды, внутренний баланс и устаревшее продление только лимита устройств с
  автоплатежом Wata не поддерживаются.
