...
Google, Яндекс, passkey, безопасное связывание совпавших OIDC-адресов и выбор отдельной почты для
уведомлений подробно описаны в разделе [«Способы входа»](login-methods.md).
...
подтверждения обеих сторон и инвалидирует старые Web App-кеши. Совпадение адреса из OIDC само по
себе не создаёт пользователя и не является разрешением на объединение.
